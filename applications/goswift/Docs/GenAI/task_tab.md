...
- [/] User Input Consolidation (Reuse)
	- [/] Create `input` feature module
	- [ ] Configure `input` module to depend on `shots` and `hydration`
	- [ ] Implement `InputUiRoute` with TabRow (reusing existing screens)
	- [ ] Update Navigation to 3-tab layout (Home, Log, Settings)
	- [ ] Update build and settings config
