...
fun EditStepForm(
    ...
    Column(modifier = uiState.modifier.padding(24.dp), verticalArrangement = Arrangement.spacedBy(28.dp)) {
        ...
        Column(modifier = Modifier.clickable { onEvent("product") }.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text(text = stringResource(R.string.applications_kocolor_features_routines_product_used_label), style = MaterialTheme.typography.labelSmall, fontWeight = FontWeight.Black, modifier = Modifier.alpha(0.4f), letterSpacing = 1.sp)
...
private fun EditStepFormPreview() {
    MaterialTheme {
        EditStepForm(
            uiState = EditStepFormUiState(RoutineStep(id = "1", title = "Step", layeringOrder = 0), emptyList()), 
            onEvent = {},
            navTo = {}
        )
    }
