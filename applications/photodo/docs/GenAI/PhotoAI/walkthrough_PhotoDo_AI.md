...
## Dark Code: Smart Capture (AI Integration)
- **AI Settings & BYOK**: Added support for user-provided Gemini API keys in the app settings. Users can now enable "AI Auto-fill" to leverage Tier 1 (Cloud) multimodal parsing.
- **Intelligent Flow**: Integrated the smart capture pipeline into the home screen camera flow. If AI is enabled, captured photos are automatically analyzed to pre-fill task details (Title, Project, Budget, etc.).
- **Visual Cues**: Implemented a "**Soft Purple**" theme for AI-enhanced task drafts to distinguish them from standard local parsing, providing clear visual feedback on the engine's capability.
- **Smart Orchestration**: Leveraged the `:features:smartcapture` module's tiered architecture to provide high-fidelity results with an API key, while maintaining a robust "Soft Blue" local fallback.
