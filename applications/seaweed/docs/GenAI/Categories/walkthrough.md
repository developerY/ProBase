...
- **Category Deletion**: Users can now delete a category directly from the "All Categories" screen via a new trash icon.
- **Confirmation Alert**: A safety dialog ensures that categories (and their associated transactions/budgets) aren't deleted accidentally.
- **Action FAB**: Added a Floating Action Button with a menu for quick tasks like adding random test data or refreshing the screen.
- **Cross-App Sync**: Deleting a category automatically synchronizes data across Home, Budget, and Analytics screens by removing all related financial records.
