...
3. **Immutable Versioned Calibration Standard**: The mathematical engine is deterministic. The parameters ($w_i$, $w_{ij}$, $\lambda$, $P_{unresolved}$, $\mu$, $\tau$) are implemented via an injectable, versioned `FashionistaCalibration` configuration derived offline from expert-rated reference ensembles.
...
5. **Weighted Measurement Coverage**: `coverage` strictly represents data completeness (how completely the outfit was measurable), calculated as $\text{coverage} = \frac{\sum w_i a_i}{\sum w_i}$. It does not represent algorithmic certainty or corrupt the raw aesthetic `score`.
...
- **`FashionistaScore`**: `data class FashionistaScore(val score: Double, val coverage: Double, val breakdown: FashionistaFeatureVector)`. Note: `score` is internally $0.000\text{--}100.000$ (UI displays rounded integer); `coverage` reflects weighted data completeness ($\frac{\sum w_i a_i}{\sum w_i}$).
...
- **`DeterministicScorer.kt`**: Computes the separated nonlinear interaction model with interaction blending parameter $\lambda$ (`interactionContribution`):
  ...
  - **Zero-Availability Fail-Safe**: If total available weight in the denominator ($\sum_i w_i a_i$) equals `0.0` (e.g., an entirely empty `FashionistaObservation` is passed), the scorer immediately short-circuits to return `FashionistaScore(score = 0.0, coverage = 0.0, breakdown = ...)` to prevent division-by-zero or `NaN` crashes.
...
2. **Weighted Evidence Completeness Test**: Verify that when facial biometrics are missing (`availability = 0.0`), `wearerIntegration` does not contribute to $Q_{base}$ or $Q_{interaction}$ numerators or denominators, and `coverage` = $\frac{\sum w_i a_i}{\sum w_i}$ decreases accordingly without penalizing `score`.
3. **Zero-Availability Fail-Safe Test**: Verify that an empty `FashionistaObservation` with zero availability across all 6 features returns `FashionistaScore(score = 0.0, coverage = 0.0)` without producing `NaN` or throwing `ArithmeticException`.
4. **Lambda Blending Invariant Test**: Assert that $Q_{base}$ and $Q_{interaction}$ are blended via $(1 - \lambda)Q_{base} + \lambda Q_{interaction}$, keeping $Q \in [0.0, 1.0]$ before penalties.
5. **Intentional Asymmetry Test**: Verify an avant-garde asymmetric silhouette with high focal hierarchy receives a strong `silhouette` score ($> 0.80$) without penalty.
6. **On-Device Downsampling Performance Test**: Verify `VisualMassEngine` completes $64 \times 128$ pixel buffer downsampling and Center of Gravity calculations in $< 5\text{ ms}$.
7. **Deterministic Replicability Test**: Assert that identical `FashionistaObservation` inputs always produce byte-for-byte identical `FashionistaScore` outputs across thread invocations.
