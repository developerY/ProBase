# Implementation Plan: Interactive Selection Mode & Deterministic-First Styling Engine

This document details the architectural plan and technical specifications for KoColor's **Interactive User Selection Mode** and **Deterministic-First Computational Styling Engine**.
...
2. **Top-K Candidate Separation:** User-locked items **do not** consume the active AI provider's Top-K retrieval budget (e.g., 3 locked items + 12 retrieved additions = 15 total items sent in prompt).
3. **Adaptive Token Preflight:** Budgeting counts the **exact assembled prompt**. If it exceeds `maxInputTokens`, the engine steps down metadata detail (`EXPANDED` $\to$ `BALANCED` $\to$ `MINIMAL`), then decrements candidate additions ($K -= 2$) while preserving locked anchors.
...
### Module 1: Domain Models & Provenance (`:features:ai:core` & `:applications:kocolor:domain`)
...
- **`ColorHarmonyEngine`**:
  - `fun calculateCompositeProfile(items: List<ClothingItem>): CompositeColorProfile`: Averages HSL values to establish dominant hue and evaluates lightness delta for contrast.
  - `fun scoreCandidateAgainstComposite(candidateHsl: Triple<Float, Float, Float>, composite: CompositeColorProfile, telemetry: StyleTelemetry): Float`: Continuous compatibility scoring ($0.0\text{--}1.0$).
...
### Module 3: Deterministic Context & Role Gap Engine (`:applications:kocolor:data:usecase`)
- **`RoleGapAnalyzer`**:
...
### Module 5: Prompt Assembler & Adaptive Waterfall Engine (`:applications:kocolor:data:usecase`)
- **`PromptAssembler`**:
  ...
  - Injects strict prompt directives: Locked items are immutable anchors; AI must complete the outfit by filling missing roles using additions without replacing locked items.
- **`StyleSimulatorEngine`**:
  ...
  - **Adaptive Step-Down Loop**:
    ...
    - Executes when prompt fits budget; cascades to next provider upon failure.
...
2. **Top-K Separation & Preflight**: Confirm that locked items do not decrement the candidate $K$ budget during preflight step-down iterations.
3. **Signature Item Bypass**: Assert that signature items (`isSignature = true`) yield a `0.0f` penalty in `RotationScoringUseCase` and are marked with `[Signature Item]` in audit logs.
4. **Privacy & App Check Audit**: Verify that cloud prompts nullify raw bitmap payloads and that App Check tokens are verified.
