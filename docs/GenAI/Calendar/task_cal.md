...
- [x] Auto-navigate Home if no categories exist in Tasks tab
    - [x] Update `photoTodoNavEntryProvider.kt` with `LaunchedEffect` for `isNoCategoriesYet`
    - [x] Verify logic and build
- [x] Add "Dark" Calendar Integration
    - [x] Create `:features:calendar` module and build configuration
    - [x] Implement `CalendarRepository` and `AndroidCalendarProvider`
    - [x] Update settings.gradle.kts to include `:features:calendar`
    - [x] Create `CalendarSyncEntity` in PhotoDo database module
    - [x] Update `PhotoDoDatabase` with new entity and AutoMigration (v3 -> v4)
    - [x] Verify module build and schema generation (4.json)
- [x] Fix "NavDisplay backstack cannot be empty" crash
    - [x] Refactor `backStack` to use atomic updates in `PhotoDoMainScreen.kt`
    - [x] Add safety checks to `onBack` and `navigateBack`
    - [x] Verify compilation and stability
