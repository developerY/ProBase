...
## Dark Code: Calendar Integration
- **Reusable Feature Module**: Created a new `:features:calendar` module to house shared logic for interacting with the Android Calendar Provider API. This module is physically isolated and ready for future use by any ProBase application.
- **Calendar Repository**: Implemented `CalendarRepository` and `AndroidCalendarProvider` to handle CRUD operations for system calendar events using `ContentResolver`.
- **Sync Metadata**: Updated the PhotoDo database to version **4** and added a `calendar_sync` table to track the relationship between tasks and system calendar events.
- **AutoMigration**: Implemented a Room `AutoMigration` (from 3 to 4) to handle the schema update.
- **Dark Code**: Maintained `internal` visibility for module-level components and omitted public DI entries to keep the feature hidden from the current release.

## Verification Summary

### Automated Tests
- Successfully ran builds for `:applications:photodo:apps:mobile:features:home`, `:applications:photodo:apps:mobile:features:tasks`, `:applications:photodo:db`, `:applications:photodo:features:timebudgeting`, and the new `:features:calendar`.
- Verified that Room generated the `2.json`, `3.json`, and `4.json` schema files.
