...
- **Hybrid AI Pipeline**:
    - **Vision (ML Kit)**: Uses Google ML Kit Text Recognition to extract raw text from images.
    - **Reasoning (Gemini Nano)**: Attempts to parse the OCR text into a structured `SmartTask` using on-device LLM.
    - **Regex Fallback**: Implemented a heuristic engine to extract core details (title, dates, budget) if the AI model is unavailable.
- **Functional UI**: Built a `SmartCaptureScreen` that allows users to capture/upload images and review the extracted task data before confirmation.
- **Feature Inventory Integration**: Added the "Smart Capture" feature to the main app's feature inventory for isolated testing and development.
